...
# Abjad 3.31 — one LilyPond file (and optional PDF/MIDI) per key.
...
SCALES = [
    ...
    (
        "Half-Dim #2 (Locrian ♮2)",
        ["C", "D", "Eb", "F", "Gb", "Ab", "Bb", "C5"],
        ["W", "H", "W", "H", "W", "W", "W"],
        "Cm7b5",
    ),
    (
...
def transpose_scale_notes(notes_spec, semitone_offset):
    return [
        abjad.NumberedPitch(numbered_pitch_from_name(n).number() + semitone_offset)
        for n in notes_spec
    ]
...
def make_bar(pitches, intervals, chord_text, scale_name):
    leaves = [
        abjad.Note.from_duration_and_pitch(abjad.Duration(1, 8), abjad.NamedPitch(p))
        for p in pitches
    ]
    while len(leaves) < 8:
        leaves.append(
            abjad.Rest.from_duration(abjad.Duration(1, 8))
        )  # pad if pentatonic/blues
    ...
    for leaf in abjad.select.leaves(container):
        if isinstance(leaf, abjad.Note):
            label = (
                "-" if i == 0 else (intervals[i - 1] if i - 1 < len(intervals) else "")
            )
            if label:
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate jazz scale charts in multiple keys."
    )
    ap.add_argument(
        "--step",
        type=int,
        default=5,
        help="Cycle step in semitones (default 5 = fourths).",
    )
    ap.add_argument(
        "--count", type=int, default=12, help="How many keys to generate (default 12)."
    )
    ap.add_argument(
        "--start", type=str, default="C", help="Starting key (e.g., C, F#, Gb, Bb)."
    )
    ap.add_argument(
        "--prefer",
        type=str,
    ...
    ap.add_argument(
        "--author",
        type=str,
        default="George K. Thiruvathukal",
        help="Author/composer name printed under the title.",
    )
    ap.add_argument(
        "--license",
        type=str,
        default="Creative Commons 4.0 International",
    ...
    for _ in range(args.count):
        score, title, key_name = build_score_for_key(
            pc, prefer, args.anchor, args.mode, args.bpm
        )
    ...
    if args.midi:
        ...
        for r in results:
            if r["midi_ok"]:
                ...
                try:
                    has_midi = "\\midi" in Path(r["ly_path"]).read_text(
