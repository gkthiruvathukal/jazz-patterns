...
`.github/workflows/deploy-web.yml` deploys the web app to **GitHub Pages** (Actions source) on pushes to `main` touching `projects/web` (and manual dispatch). Live at **https://gkt.sh/jazz-patterns/** (served under the user's verified `gkt.sh` domain). `vite.config.ts` sets `base: "./"` so assets resolve under the `/jazz-patterns/` subpath.
